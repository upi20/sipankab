...
class Tahapan extends Model
{
    ...
    protected $table = 'import_tahapan';
    const tableName = 'import_tahapan';
    const excelFolder = 'assets/import/tahapan';
    ...
    {
        return $this->hasMany(ModelsTahapan::class, 'import_id', 'id');
    }
    ...
    public static function import(Request $request, Tahapan $model)
    {
        // setup
        ...
        foreach ($array_from_excel as $i => $v) {
            ...
            // cek
            $kode = $v[1];
            $cek = ModelsTahapan::where('kode', $kode)->count();
            if ($cek > 0) return [
                'status' => false,
                'error' => response()->json(['message' => "Kode $kode Sudah di gunakan, Silahkan cek data nomor {$v[0]}"], 400),
                'excel' => $file_excel
            ];
            $tahapan = new ModelsTahapan();
            $tahapan->kode = $kode;
            $tahapan->nama = $v[2];
            $tahapan->bobot = $v[3];
            $tahapan->import_id = $model->id;
            $tahapan->save();
            $count++;
        }
    ...
    public static function format(Request $request)
    {
        ...
        $title_excel = "Formulir Import Tahapan";
        // Header excel ================================================================================================
        ...
        $sheet->mergeCells($col_start . $row . ":" . $col_end . $row)
            ->setCellValue("A$row", "Formulir Import Tahapan");
    ...
    {
        // data body
        $details = ModelsTahapan::orderBy('kode')->get();
        $bulan_array = [
            1 => 'Januari',
            2 => 'February',
            3 => 'Maret',
        ...
        $col_start = "A";
        $col_end = chr(64 + count($headers));
        $title_excel = "Export Semua Data Tahapan";
        // Header excel ================================================================================================
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        ...
        $row += 1;
        $sheet->mergeCells($col_start . $row . ":" . $col_end . $row)
            ->setCellValue("A$row", "Export Semua Data Tahapan");
        $sheet->getStyle($col_start . $row . ":" . $col_end . $row)->applyFromArray([
            "font" => [
                "bold" => true,
                "size" => 13
